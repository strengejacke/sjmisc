...
#'
#' @return A numeric vector with all random intercept intraclass-correlations.
#'
#' @references \itemize{
...
icc.lme4 <- function(fit) {
  ...
  if (any(class(fit) == "glmerMod") || any(class(fit) == "lmerMod") || any(class(fit) == "merModLmerTest")) {
    # ------------------------
    # Formular to calculate ICC taken from CrossValidated:
    # http://stats.stackexchange.com/questions/115526/intraclass-correlation-coefficient-in-mixed-model-with-random-slopes
    # ------------------------
