...
Tidying up, transforming and exploring data is an important part of data analysis, and you can manage many common tasks in this process with the [tidyverse](http://tidyverse.org/) or related packages. The **sjmisc**-package fits into this workflow, especially when you work with [labelled data](labelleddata.html), because it offers functions for data transformation and labelled data utility functions. This vignette describes typical steps when beginning with data exploration.
...
By default, it looks for variable name and labels. The function also supports regex-patterns. By default, `find_var()` returns the column-indices, but you can also print a small "summary"" with the `as.varlab`-argument.
