...
The selection of variables specified in the `...`-ellipses-argument is powered by dplyr's `select()`. This means, you can use existing functions like `:` to select a range of variables, or also use dplyr's `select_helpers`, like `contains()` or `one_of()`.
